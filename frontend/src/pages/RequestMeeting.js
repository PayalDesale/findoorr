...
const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .rm-slot.unavailable { opacity: 0.4; cursor: not-allowed; }
  ...
  .rm-submit-btn:active { transform: translate(1px,1px); box-shadow: 2px 2px 0px #c0570e; }
  ...
  .rm-success-btn:hover { transform: translate(-2px,-2px); box-shadow: 6px 6px 0px #c0570e; }
...
const professors = [
  { emoji: '👨‍💻', name: 'Dr. R. Sharma', dept: 'Computer Science', status: 'free' },
  { emoji: '👩‍🔬', name: 'Prof. A. Kulkarni', dept: 'Artificial Intelligence', status: 'busy' },
  { emoji: '👨‍🏫', name: 'Dr. V. Mehta', dept: 'Database Systems', status: 'free' },
  { emoji: '👩‍💼', name: 'Prof. S. Desai', dept: 'Software Engineering', status: 'free' },
];

const dates = [
  { name: 'MON', num: 19 }, { name: 'TUE', num: 20 }, { name: 'WED', num: 21 },
  { name: 'THU', num: 22 }, { name: 'FRI', num: 23 },
];

const timeSlots = [
  { time: '9:00 AM', avail: 'Available' }, { time: '10:00 AM', avail: 'Available' },
  { time: '11:00 AM', avail: 'Busy', unavail: true }, { time: '12:00 PM', avail: 'Available' },
  { time: '2:00 PM', avail: 'Available' }, { time: '3:30 PM', avail: 'Available' },
];
          ...
          {step === 1 && (
            <div className="rm-card">
              ...
              <div className="rm-prof-grid">
                {professors.map((p, i) => (
                  <div key={p.name} className={`rm-prof-option ${selectedProf === i ? 'selected' : ''}`}
                    onClick={() => setSelectedProf(i)}>
                    <span className="rm-prof-emoji">{p.emoji}</span>
                    <div>
                      <div className="rm-prof-opt-name">{p.name}</div>
                      <div className="rm-prof-opt-dept">{p.dept}</div>
                    </div>
                    <span className={`rm-prof-opt-status ${p.status}`}>{p.status === 'free' ? 'Free' : 'Busy'}</span>
                  </div>
                ))}
              </div>
          ...
          {step === 2 && (
            <div className="rm-card">
              ...
              <div className="rm-date-strip">
                {dates.map((d, i) => (
                  <div key={d.name} className={`rm-date-day ${selectedDate === i ? 'active' : ''}`}
              ...
              <div className="rm-slot-grid">
                {timeSlots.map((s, i) => (
                  <div key={s.time}
                    className={`rm-slot ${selectedSlot === i ? 'selected' : ''} ${s.unavail ? 'unavailable' : ''}`}
                    onClick={() => !s.unavail && setSelectedSlot(i)}>
                    <div className="rm-slot-time">{s.time}</div>
                    <div className="rm-slot-avail">{s.avail}</div>
                  </div>
                ))}
              </div>
          ...
          {step === 4 && (
            <div className="rm-card">
              ...
              <div style={{marginBottom:'24px'}}>
                {[
                  { key: 'Professor', val: `${professors[selectedProf].emoji} ${professors[selectedProf].name}` },
                  { key: 'Date', val: `${dates[selectedDate].name}, May ${dates[selectedDate].num}` },
              ...
              <div className="rm-nav-btn-row">
                ...
                <button className="rm-next-btn" onClick={() => setSubmitted(true)}>send request 🎉</button>
              </div>
            </div>
          )}
